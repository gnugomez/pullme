...
import { PRReviewReminderController } from './PRReviewReminder';
...
async function main() {
  ...
  log.level = parseInt(options.logLevel);
  ...
  await runPeriodically(() => prReminder.run(), parseInt(options.pollingInterval) * 1000);
}
